...
function Cardapio() {
  ...
  useEffect(() => {
    // Substituir o uso do lanchesData por uma chamada ao backend
    axios
      .get("http://localhost:3001/lanches") // Endpoint do backend
      .then((response) => setLanches(response.data))
      .catch((error) =>
          ...
          {lanches
            ...
            .map((lanche, index) => (
              <button
                ...
                className={`produtos0${index + 1}`}
              >
                <div key={lanche.id}>
                  <p className="lancamentonome">{lanche.nome}</p>
                  <img
                    src={lanche.imagem}
                    className="lancamentoImg"
        ...
        >
          <Box sx={style}>
            ...
            <div className="modal_body">
              <div className="title_modal">{selectedLanche.nome}</div>
